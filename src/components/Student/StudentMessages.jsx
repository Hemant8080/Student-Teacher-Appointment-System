...
const StudentMessages = () => {
  ...
  const loadMessages = async () => {
    ...
    setError('');
    try {
      const result = await getMessagesForUser(user.uid, 'student');
      if (result.success) {
          ...
          <div className="text-center py-8">
            ...
            <p className="text-sm text-gray-400 mt-2">
              {appointments.length === 0 
                ? 'You need approved appointments before you can send messages to teachers.'
                    ...
                    >
                      <div className="flex justify-between items-start mb-1">
                        <span className="text-sm font-medium text-gray-900">
                          {message.senderType === 'student' ? 'You' : 'Teacher'}
                        </span>
      ...
      {appointments.length === 0 && (
        <div className="bg-blue-50 border border-blue-200 rounded-lg p-4">
          ...
          <p className="text-sm text-blue-800">
            You need to have approved appointments before you can send messages to teachers.
            Go to the "Find Teachers" tab to book appointments.
          </p>
        </div>
      )}
    </div>
